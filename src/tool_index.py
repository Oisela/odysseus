...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "manage_notes": "Create and manage notes and checklists (Google Keep-style). ALWAYS use this for note/todo/checklist/reminder creation — NEVER hit /api/notes via app_api. Accepts natural-language `due_date` like 'tomorrow at 9am' or '11pm today' (parsed in the USER'S timezone). The due_date IS the reminder — it fires a notification at that time, so do NOT also create a calendar event for the same reminder. Set colors, labels, pin, archive. `label` = the note's list in the Notes sidebar; shopping-list requests = one todo per item with label 'Einkauf'; recipes = note_type 'recipe' (instructions in content, ingredients as checklist items, label 'Recipes'). Do NOT use manage_memory for note content.",
