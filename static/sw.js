...
// Strategy:
//   - HTML (navigation): network-first, cache fallback for offline. The old
//     stale-while-revalidate served a CACHED shell while modules loaded
//     network-first — after every version switch (update, beta sync,
//     rollback) one load ran a mixed old-shell/new-modules app that failed
//     in weird ways ("Failed to load" cards, 2026-07-16). Correctness over
//     instant-open: the shell is small, and offline still works via cache.
...
self.addEventListener('fetch', (e) => {
  ...
  // HTML navigation: network-first for the SPA root so the shell ALWAYS
  // matches the server's current version; cached copy only when offline.
  // Other navigations (e.g. a deep-linked /static/*.html page) go to the
