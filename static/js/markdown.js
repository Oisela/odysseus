...
export function mdToHtml(src, opts) {
  ...
  if (window.katex) {
    ...
    // preceded by a non-space and not followed by a digit. The rule lives in
    // INLINE_MATH_MD_SOURCE so the notes rich editor shares exactly one grammar.
...
export const UPLOAD_IMAGE_MD_SOURCE = '!\\[[^\\]]*\\]\\((\\/api\\/upload\\/[A-Za-z0-9_-]+(?:\\?[^\\s)]*)?)\\)';
